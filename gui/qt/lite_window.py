...
try:
    ...
    print "You need to have PyQT installed to run Electrum-ppc in graphical mode."
...
from electrum_ppc.util import get_resource_path as rsrc
from electrum_ppc.bitcoin import is_valid
...
import re
import time
from electrum_ppc.wallet import Wallet, WalletStorage
import webbrowser
import history_widget
import receiving_widget
from electrum_ppc import util
import datetime

from electrum_ppc.version import ELECTRUM_VERSION as electrum_version
from electrum_ppc.util import format_satoshis, age
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a Peercoin address or contact"))
        ...
        self.setWindowTitle("Electrum-ppc")
    ...
    def set_balances(self, btc_balance):
        """Set the Peercoin balance and update the amount label accordingly."""
        ...
        self.setWindowTitle("Electrum-ppc %s - %s %s" % (electrum_version, amount, unit))

    def amount_input_changed(self, amount_text):
        """Update the number of Peercoins displayed."""
    ...
    def check_button_status(self):
        """Check that the Peercoin address is valid and that something
    ...
    def the_website(self):
        webbrowser.open("http://ppc.electrum-alt.org")
...
class BalanceLabel(QLabel):
    ...
    def set_balance_text(self, amount, unit, quote_text):
        """Set the amount of Peercoins in the gui."""
        if self.state == self.SHOW_CONNECTING:
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your Peercoin address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum-ppc - " + _("Receive Peercoin payment"))
...
class MiniActuator:
    ...
    def send(self, address, amount, parent_window):
        """Send Peercoins to the target address."""
