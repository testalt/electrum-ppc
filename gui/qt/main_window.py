...
from electrum_ppc.i18n import _, set_language
from electrum_ppc.util import print_error, print_msg
...
from electrum_ppc.bitcoin import MIN_RELAY_TX_FEE, is_valid
from electrum_ppc.plugins import run_hook

import icons_rc

from electrum_ppc.util import format_satoshis
from electrum_ppc import Transaction
from electrum_ppc import mnemonic
from electrum_ppc import util, bitcoin, commands, Interface, Wallet
from electrum_ppc import SimpleConfig, Wallet, WalletStorage
from electrum_ppc import Imported_Wallet
...
from electrum_ppc import ELECTRUM_VERSION
import re
...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/electrum-ppc.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        import electrum_ppc as electrum
        self.wallet = wallet
        ...
        title = 'Electrum-ppc ' + self.wallet.electrum_version + '  -  ' + self.wallet.storage.path
    ...
    def backup_wallet(self):
        ...
        if new_path != path:
            try:
                ...
                QMessageBox.critical(None,"Unable to create backup", _("Electrum-ppc was unable to copy your wallet file to the specified location.")+"\n" + str(reason))
    ...
    def init_menubar(self):
        ...
        tools_menu.addAction(_("Electrum-ppc preferences") if sys.platform == 'darwin' else _("Preferences"), self.settings_dialog)
        ...
        help_menu.addAction(_("&Official website"), lambda: webbrowser.open("http://ppc.electrum-alt.org"))
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum-ppc",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum-ppc's focus is speed, with low resource usage and simplifying Peercoin. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Peercoin system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Electrum-ppc - " + _("Reporting Bugs"),
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Electrum-ppc", message, QSystemTrayIcon.Information, 20000)
    ...
    def update_receive_qr(self):
        ...
        if addr:
            ...
            p = urlparse.ParseResult(scheme='peercoin', netloc='', path=addr, params='', query='&'.join(query), fragment='')
    ...
    def create_send_tab(self):
        ...
        self.payto_help = HelpButton(_('Recipient of the funds.') + '\n\n' + _('You may enter a Peercoin address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Peercoin address)'))
        ...
        msg = _('Peercoin transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def read_send_tab(self):
        ...
        for type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('Peercoin Address is None'), _('OK'))
                return
            if type == 'op_return':
                continue
            if type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid Peercoin Address'), _('OK'))
                return
            if amount is None:
    ...
    def pay_from_URI(self,URI):
        ...
        try:
            ...
            QMessageBox.warning(self, _('Error'), _('Invalid peercoin URI:') + '\n' + str(e), _('OK'))
            return
        ...
        from electrum_ppc import paymentrequest
        def payment_request():
    ...
    def create_contact_menu(self, position):
        ...
        if not item:
            ...
            menu.addAction(_("QR code"), lambda: self.show_qrcode("peercoin:" + addr, _("Address")))
            if is_editable:
    ...
    def show_invoice(self, key):
        from electrum_ppc.paymentrequest import PaymentRequest
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive peercoins.") + " " \
    ...
    def tx_from_text(self, txt):
        ...
        if is_hex:
            try:
                ...
                traceback.print_exc(file=sys.stdout)
                QMessageBox.critical(None, _("Unable to parse transaction"), _("Electrum-ppc was unable to parse your transaction"))
                return
    ...
    def read_tx_from_qrcode(self):
        from electrum_ppc import qrscanner
        try:
    ...
    def read_tx_from_file(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to read file or no transaction found"), _("Electrum-ppc was unable to open your transaction file") + "\n" + str(reason))
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'electrum-ppc-private-keys.csv'
        ...
        try:
            ...
            export_error_label = _("Electrum-ppc was unable to produce a private key-export.")
    ...
    def do_import_labels(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to import labels"), _("Electrum-ppc was unable to import your labels.")+"\n" + str(reason))


    def do_export_labels(self):
        labels = self.wallet.labels
        try:
            fileName = self.getSaveFileName(_("Select file to save your labels"), 'electrum_ppc_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
                QMessageBox.information(None, _("Labels exported"), _("Your labels where exported to")+" '%s'" % str(fileName))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to export labels"), _("Electrum-ppc was unable to export your labels.")+"\n" + str(reason))
    ...
    def export_history_dialog(self):
        ...
        defaultname = os.path.expanduser('~/electrum-ppc-history.csv')
        ...
        try:
            ...
            export_error_label = _("Electrum-ppc was unable to produce a transaction export.")
    ...
    def settings_dialog(self):
        ...
        d = QDialog(self)
        d.setWindowTitle(_('Electrum-ppc Settings'))
        d.setModal(1)
        vbox = QVBoxLayout()
        grid = QGridLayout()
        ...
        from electrum_ppc.i18n import languages
        ...
        def on_unit(x):
            ...
            elif unit_result == 'mPPC':
                self.decimal_point = 5
            elif unit_result == 'pbits':
        ...
        if self.need_restart:
            QMessageBox.warning(self, _('Success'), _('Please restart Electrum-ppc to activate the new GUI settings'), _('OK'))



    def run_network_dialog(self):
        if not self.network:
            QMessageBox.warning(self, _('Offline'), _('You are using Electrum-ppc in offline mode.\nRestart Electrum-ppc if you want to get connected.'), _('OK'))
            return
    ...
    def plugins_dialog(self):
        from electrum_ppc.plugins import plugins

        d = QDialog(self)
        d.setWindowTitle(_('Electrum-ppc Plugins'))
        d.setModal(1)
