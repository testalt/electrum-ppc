...
from electrum_ppc.util import print_error
from electrum_ppc.bitcoin import is_valid
from electrum_ppc import WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-ppc"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_ppc.util import format_satoshis, parse_URI
from electrum_ppc.network import DEFAULT_SERVERS
from electrum_ppc.bitcoin import MIN_RELAY_TX_FEE
...
def restore_create_dialog():
    ...
    dialog = Gtk.Dialog("electrum-ppc", parent=None, 
                        flags=Gtk.DialogFlags.MODAL,
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Electrum-ppc server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-ppc ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid peercoin address:\n"+to_address)
            return
        ...
        #@todo Peercoin electrum-ltc modifies this...
